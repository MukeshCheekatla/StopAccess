...
export async function renderDashboard(container) {
  ...
  try {
    ...
    if (syncStatus === 'connected') {
      ...
      shieldSub = `${(syncMode as string).toUpperCase()} · Last sync ${fmtDate(
        lastSync,
      )}`;
    ...
    container.innerHTML = `
      ...
      <div class="stats-row">
        <div class="stat-card">
          <div class="stat-val">${fmtTime(allTotalMs as number) || '0m'}</div>
    ...
    container.querySelector('#wb_apps')?.addEventListener('click', () => {
      (document.querySelector('[data-tab="apps"]') as HTMLElement)?.click();
    });
    container
