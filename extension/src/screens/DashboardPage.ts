...
export async function renderDashboardPage(container) {
  ...
  try {
    ...
    const allTotalMs: number = Object.values(usageAny).reduce(
      (a: number, b: any) => a + (b.time || 0),
      0,
    ) as number;
    const domainList = Object.entries(usageAny)
    ...
    if (syncStatus === 'connected') {
      try {
        const countersRes = await (nextDNSApi as any).getAnalyticsCounters();
