...
export async function renderInsightsPage(container) {
  ...
  try {
    const snapshots = ((await storage.getArray('fg_snapshots')) || []) as any[];
    ...
    if (isConfigured) {
      try {
        ...
        if ((logsRes as any).ok) {
          blockedLogs = (logsRes as any).data.filter(
            (l) => l.status === 'blocked',
          );
        }
        if ((domainsRes as any).ok) {
          topBlocked = (domainsRes as any).data.slice(0, 5);
        }
      } catch (e) {
